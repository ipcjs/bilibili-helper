...
function AcfunParser(a) {
    ...
    for (var e = 0; e < c.length; e++) {
        var f = {}, g = c[e].c.split(",");
        if (g.length > 0) {
            if (f.stime = 1e3 * parseFloat(g[0]), f.color = parseInt(g[1]), f.mode = parseInt(g[2]), f.size = parseInt(g[3]), f.hash = g[4], f.date = parseInt(g[5]), f.position = "absolute", 7 != f.mode ? (f.text = c[e].m.replace(/(\/n|\\n|\n|\r\n|\\r)/g, "\n"), f.text = f.text.replace(/\r/g, "\n"), f.text = f.text.replace(/\s/g, " ")) : f.text = c[e].m, 7 == f.mode) {
                try {
                ...
                if (f.position = "relative", f.text = h.n, f.text = f.text.replace(/\ /g, " "), null != h.a ? f.opacity = h.a : f.opacity = 1, null != h.p ? (f.x = h.p.x / 1e3, f.y = h.p.y / 1e3) : (f.x = 0, f.y = 0), f.shadow = h.b, f.dur = 4e3, null != h.l && (f.moveDelay = 1e3 * h.l), null != h.z && h.z.length > 0) {
                    ...
                    for (var i = 0, j = {
                        ...
                        i += l;
                        var m = {
                            x: {from: j.x, to: h.z[k].x / 1e3, dur: l, delay: 0},
                            y: {from: j.y, to: h.z[k].y / 1e3, dur: l, delay: 0}
                        };
...
function BilibiliParser(a, b, c) {
    ...
    if (null !== a)var e = a.getElementsByTagName("d"); else {
    ...
    for (var g = [], h = 0; h < e.length; h++)if (null != e[h].getAttribute("p")) {
        ...
        var b = e[h].childNodes[0].nodeValue, j = {};
        if (j.stime = Math.round(1e3 * parseFloat(i[0])), j.size = parseInt(i[2]), j.color = parseInt(i[3]), j.mode = parseInt(i[1]), j.date = parseInt(i[4]), j.pool = parseInt(i[5]), j.position = "absolute", null != i[7] && (j.dbid = parseInt(i[7])), j.hash = i[6], j.border = !1, j.mode < 7)j.text = b.replace(/(\/n|\\n|\n|\r\n)/g, "\n"); else if (7 == j.mode)try {
            if (adv = JSON.parse(d(b)), j.shadow = !0, j.x = parseFloat(adv[0]), j.y = parseFloat(adv[1]), (Math.floor(j.x) < j.x || Math.floor(j.y) < j.y) && (j.position = "relative"), j.text = adv[4].replace(/(\/n|\\n|\n|\r\n)/g, "\n"), j.rZ = 0, j.rY = 0, adv.length >= 7 && (j.rZ = parseInt(adv[5], 10), j.rY = parseInt(adv[6], 10)), j.motion = [], j.movable = !1, adv.length >= 11) {
                j.movable = !0;
                var k = 500, l = {
                    x: {from: j.x, to: parseFloat(adv[7]), dur: k, delay: 0},
                    y: {from: j.y, to: parseFloat(adv[8]), dur: k, delay: 0}
                };
                if ("" !== adv[9] && (k = parseInt(adv[9], 10), l.x.dur = k, l.y.dur = k), "" !== adv[10] && (l.x.delay = parseInt(adv[10], 10), l.y.delay = parseInt(adv[10], 10)), adv.length > 11 && (j.shadow = adv[11], "true" === j.shadow && (j.shadow = !0), "false" === j.shadow && (j.shadow = !1), null != adv[12] && (j.font = adv[12]), adv.length > 14)) {
                    ...
                    for (var m = adv[14], n = {
                        ...
                        switch (r[1]) {
                            ...
                            case"L":
                                o.push({
                                    x: {from: n.x, to: parseInt(r[2], 10), dur: k / q, delay: 0},
                                    y: {from: n.y, to: parseInt(r[3], 10), dur: k / q, delay: 0}
                                }), n.x = parseInt(r[2], 10), n.y = parseInt(r[3], 10)
                        }
                        r = p.exec(m)
                    }
            ...
            if (null != f && f.length > 1) {
                var s = parseFloat(f[0]), t = parseFloat(f[1]);
                j.opacity = s, s !== t && (j.alpha = {from: s, to: t})
            }
        } catch (u) {
...
var BinArray = function () {
    ...
    function a(a, b) {
        "undefined" == typeof a && (a = 0), "undefined" == typeof b && (b = 0), this._pools = [[]], this.avoid = 1, this._width = a, this._height = b
    }
    ...
    function a(b, c) {
        if ("undefined" == typeof c && (c = {}), this.mode = 1, this.stime = 0, this.text = "", this.ttl = 5e3, this.dur = 5e3, this.cindex = -1, this.motion = [], this.movable = !0, this._alphaMotion = null, this.absolute = !0, this.align = 0, this._alpha = 1, this._size = 25, this._color = 16777215, this._border = !1, this._shadow = !0, this._font = "", !b)throw new Error("Comment not bound to comment manager.");
        if (this.parent = b, c.hasOwnProperty("stime") && (this.stime = c.stime), c.hasOwnProperty("mode") ? this.mode = c.mode : this.mode = 1, c.hasOwnProperty("dur") && (this.dur = c.dur, this.ttl = this.dur), this.dur *= this.parent.options.global.scale, this.ttl *= this.parent.options.global.scale, c.hasOwnProperty("text") && (this.text = c.text), c.hasOwnProperty("motion")) {
    ...
    return a.prototype.init = function (a) {
        "undefined" == typeof a && (a = null), null !== a ? this.dom = a.dom : this.dom = document.createElement("div"), this.dom.className = this.parent.options.global.className, this.dom.appendChild(document.createTextNode(this.text)), this.dom.textContent = this.text, this.dom.innerText = this.text, this.dom.setAttribute("content", this.text), this.size = this._size, 16777215 != this._color && (this.color = this._color), this.shadow = this._shadow, this._border && (this.border = this._border), "" !== this._font && (this.font = this._font), void 0 !== this._x && (this.x = this._x), void 0 !== this._y && (this.y = this._y), (1 !== this._alpha || this.parent.options.global.opacity < 1) && (this.alpha = this._alpha), this.motion.length > 0 && this.animate()
        ...
        if (this._alphaMotion && (this.alpha = (this.dur - this.ttl) * (this._alphaMotion.to - this._alphaMotion.from) / this.dur + this._alphaMotion.from), 0 !== this.motion.length) {
            var a = Math.max(this.ttl, 0), b = this.dur - a - this._motionStart[this._curMotion];
...
}(CoreComment), __extends = this.__extends || function (a, b) {
        function c() {
            this.constructor = a
        }
...
}(CoreComment), CSSCompatLayer = function () {
    function a() {
    }

    return a.transform = function (a, b) {
        a.style.transform = b, a.style.webkitTransform = b, a.style.msTransform = b, a.style.oTransform = b
    }, a
}(), CSSScrollComment = function (a) {
    function b() {
        a.apply(this, arguments), this._dirtyCSS = !0
    }

    return __extends(b, a), Object.defineProperty(b.prototype, "x", {
        get: function () {
            return this.ttl / this.dur * (this.parent.width + this.width) - this.width
        }, set: function (a) {
            if ("number" == typeof this._x) {
                var b = a - this._x;
                this._x = a, CSSCompatLayer.transform(this.dom, "translateX(" + b + "px)")
            } else this._x = a, this.absolute || (this._x *= this.parent.width), this.align % 2 === 0 ? this.dom.style.left = this._x + "px" : this.dom.style.right = this._x + "px"
        }, enumerable: !0, configurable: !0
    }), b.prototype.update = function () {
        this._dirtyCSS && (this.dom.style.transition = "transform " + this.ttl + "ms linear", this.x = -this.width, this._dirtyCSS = !1)
    }, b.prototype.invalidate = function () {
        a.prototype.invalidate.call(this), this._dirtyCSS = !0
    }, b.prototype.stop = function () {
        this.dom.style.transition = "", this.x = this._x, this._x = null, this.x = this.ttl / this.dur * (this.parent.width + this.width) - this.width, this._dirtyCSS = !0
    }, b
}(ScrollComment), CommentManager = function () {
    function a(a) {
        var b = 0;
        ...
        }, this.pauseComment = function () {
            Array.prototype.slice.call(this.stage.children).forEach(function (a) {
                a.classList.contains("cmt") && !a.classList.contains("paused") && (a.finalTransform = a.style.transform, a.style.transform = window.getComputedStyle(a).getPropertyValue("transform"), a.finalDuration = parseFloat(a.style.transitionDuration) - (new Date).getTime() + a.transitionStartTime, a.style.transitionDuration = "10ms", a.classList.add("paused"))
            })
        }, this.resumeComment = function () {
            Array.prototype.slice.call(this.stage.children).forEach(function (a) {
                a.classList.contains("cmt") && a.classList.contains("paused") && (a.style.transitionDuration = a.finalDuration + "ms", a.style.transform = a.finalTransform, a.transitionStartTime = (new Date).getTime(), a.classList.remove("paused"))
            })
        }
    }
    ...
    return a.prototype.stop = function () {
        this.stopTimer();
        for (var a = 0; a < this.runline.length; a++)"undefined" != typeof this.runline[a].stop && this.runline[a].stop()
        ...
        this.width = this.stage.offsetWidth, this.height = this.stage.offsetHeight, this.options.global.scale = this.width / 680, this.dispatchEvent("resize");
        ...
        if (null == this.filter || (a = this.filter.doModify(a), null != a)) {
            if (1 === a.mode || 2 === a.mode || 6 === a.mode)var c = new CSSScrollComment(this, a); else var c = new CoreComment(this, a);
            switch (c.mode) {
                case 1:
                    c.align = 0;
                    break;
            ...
            switch (c.init(), this.stage.appendChild(c.dom), c.dom.transitionStartTime = (new Date).getTime(), c.mode) {
                default:
                case 1:
