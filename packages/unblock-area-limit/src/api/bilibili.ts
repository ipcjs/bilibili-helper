...
export class BiliBiliApi {
    ...
    getSeasonInfoByEpIdOnThailand(ep_id: string, season_id: string) {
        return Async.ajax<SeasonInfoOnThailand>(`${this.server}/intl/gateway/v2/ogv/view/app/season?` + (ep_id != '' ? `ep_id=${ep_id}` : `season_id=${season_id}`) + '&s_locale=zh_SG')
    }
}
